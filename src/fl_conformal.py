...
def cal_conform_scores(output, gt):
    ...
    output = output.exp()
    for j in range(num):
        ind = (output[j] > output[j, gt[j]]).long()
        # print(output[j])
        # print(ind)
        # print(output[j]*ind)
        res[j] += torch.sum(output[j]*ind)
        res[j] += np.random.uniform(0,1) * output[j, gt[j]]
    return res
...
def score2vec(scores, vec_dim, scores2vector):
    if scores2vector=='histogram':
        ...
        for i in range(1,vec_dim):
            ...
            vec[i] = cur
        return vec
    elif scores2vector=='kmeans':
        from sklearn.cluster import KMeans
        scores = np.resize(scores,(len(scores),1))
        # print(scores)
        kmeans = KMeans(n_clusters=10, random_state=0,copy_x=False).fit(scores)
...
def cal_distance(vec1,vec2):
    ...
    # return np.max(np.sqrt((vec1 - vec2)**2))
...
if __name__ == '__main__':
    ...
    if args.dataset != 'tinyimagenet':
        ...
        if iid_>1:
            iid_=1
        global_model = torch.load('./save/global_model_{}_{}_{}_{}_{}_{}_{}'.format(args.dataset, args.model, args.epochs, args.frac, iid_, args.local_ep, args.local_bs))
    else:
        if args.iid==1:
    ...
    if args.iid==1:
        ...
        # for plot bound
        cali_set = train_dataset
        num_ben = int(0.8 * len(cali_set))
        print(f'length of minimal benign sample size: {num_ben}')
        cali_data_split = []
        for u in range(args.num_users):
            cali_data_split.append(torch.utils.data.random_split(cali_set, [len(cali_set) - num_ben, num_ben])[1])
        cali_loader = [torch.utils.data.DataLoader(x, batch_size=128, shuffle=True) for x in cali_data_split]
    ...
    print('Under attack {} with num_malicious_clients {} with robust method {} with iid {}, communication cost and approximation error of the quantile: {} / {:4f}, avg_coverage and avg_set_size: {:4f} / {:4f}'.format(args.attack_type, args.num_malicious_clients, args.robust_conformal, args.iid, communication_cost, approx_error, 1.0 * num_covered / num_total, 1.0 * num_setsize/num_total))
